...
First, the **res** parameter take as input the object resulting from the analysis you wish to produce a reporting from.  `Yes_MCA` will take the result of a MCA, `Yes_decat`the result of a decat (a function of SensoMineR), etc.  
...
**Size_tab** is used to set the maximum number of rows a table may have per slide.   Lastly, the **proba** parameter is used to set the significance threshold.
<br><br />

The **x1** and **x2** (in Yes_PCA/MCA/decat) parameters let you choose which dimensions to plot and describe.  
In Yes_decat and Yes_textual, the **col.neg** and **col.pos** arguments set colors for the color gradient applied on the word clouds. By default, these colors are set to red and blue.
